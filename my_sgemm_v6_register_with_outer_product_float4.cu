  ...
  每 thread 加载 4 → 总量刚好够 → ✅ 不需要 for
    ...
    */        
        for(int i = 0; i < M_NUM_PER_THREAD; i++) {
            ...
            // 对于a_shared 来说，64×64的block，y维度被分成 M_NUM_PER_THREAD均等的份，向下移动了ty个单位，
            // 也就是ty * M_NUM_PER_THREAD, 然后再偏移i个单位
            // x方向上，K_NUM_PER_BLOCK被分成了K_NUM_PER_THREAD的均等份，并且移动了tx个单位
            // 并且由于x方向已经是向量化加载4个数据了，所以不需要是使用偏置
                FETCH_FLOAT4(A_ptr_start[K * (ty * M_NUM_PER_THREAD + i) + tx * K_NUM_PER_THREAD + s]);
            // 对于A_ptr_start来说，此时应该从整个矩阵的角度分析，K方向上被分成了以K_NUM_PER_BLOCK均等的份
            // 向下移动 ty * M_NUM_PER_THREAD + i个单位，每个单位的步长为K
        }
...
int main(){
    ...
    float* matrix_C_host_cublas = (float*)malloc(mem_size_C);
    float* matrix_C_device_cublas;
    cudaMalloc(&matrix_C_device_cublas, mem_size_C);
    ...
    cudaEventElapsedTime(&cublas_time, start_cublas, stop_cublas);
    printf("cuBLAS SGEMM time: %.6f ms\n", cublas_time);
    
    cudaMemcpy(matrix_C_host_cublas, matrix_C_device_cublas, mem_size_C, cudaMemcpyDeviceToHost);
    
    // optional: check correctness vs CPU
    float diff_cublas = compare_matrices(m, n, matrix_C_host_cublas, matrix_C_host_cpu_calc, n, n);
    printf("cuBLAS diff vs CPU: %f\n", diff_cublas);
    
    cudaFree(matrix_C_device_cublas);
    free(matrix_C_host_cublas);
    ...
    float my_gflops = gflops / (milliseconds / 1000.f);
    float cublas_gflops = gflops / (cublas_time / 1000.f);
    
    printf("[Perf] MyKernel GFLOPs: %.2f\n", my_gflops);
    printf("[Perf] cuBLAS GFLOPs: %.2f\n", cublas_gflops);
    printf("[Perf] Efficiency: %.2f%% of cuBLAS\n", 100.f * my_gflops / cublas_gflops);
